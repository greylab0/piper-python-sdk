...
class PiperClient:
    ...
    def __init__(self,
        ...
        sdk_version = "0.2.0" # Update this for new release
        ...
        if auto_discover_instance_id:
             self.discover_local_instance_id() # This line is now correctly indented

    # --- ALL SUBSEQUENT METHODS MUST BE INDENTED TO BE PART OF THE CLASS ---
    ...
    def discover_local_instance_id(self, force_refresh: bool = False) -> Optional[str]:
        ...
        try:
            ...
            self._discovered_instance_id = None
            return None
        except Exception as e: # Broader catch for JSONDecodeError, HTTPError etc.
    ...
    def _resolve_piper_variable(self, variable_name: str, instance_id_for_context: str) -> str:
        ...
        try:
            ...
            if 400 <= response.status_code < 600:
                ...
                if response.status_code == 404 or error_code == 'mapping_not_found':
                    raise PiperAuthError(f"No active grant mapping found for var '{trimmed_variable_name}' for instance '{instance_id_for_context}'.", status_code=404, error_code='mapping_not_found', error_details=error_details)
        ...
        except (PiperAuthError, ValueError):
            raise
        except requests.exceptions.RequestException as e:
            ...
            logger.error(f"Unexpected error resolving variable: {e}", exc_info=True)
    ...
    def _fetch_piper_sts_token(self, credential_ids: List[str], instance_id_for_context: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 400 <= response.status_code < 600:
                ...
                if response.status_code == 401 or error_code == 'invalid_token':
                    self._token_expiries[(target_audience, instance_id_for_context)] = 0
                    raise PiperAuthError(f"Agent auth failed for scoped creds: {error_description}", status_code=401, error_code=error_code or 'invalid_token', error_details=error_details)
                if response.status_code == 403 or error_code == 'permission_denied':
                    raise PiperAuthError(f"Permission denied for scoped creds: {error_description}", status_code=403, error_code=error_code or 'permission_denied', error_details=error_details)
            ...
            logger.error(f"Unexpected error getting scoped creds by ID: {e}", exc_info=True)
    ...
    def get_secret(self,
        ...
        piper_sts_response = None
        piper_error_encountered = None
        ...
        try:
            ...
            return {
                ...
                "piper_credential_id": piper_sts_response.get('granted_credential_ids', [credential_id])[0], # Use first granted or resolved
            ...
            piper_error_encountered = e
            logger.warning(f"Piper SDK configuration error for '{variable_name}': {e}")
        except Exception as e:
            piper_error_encountered = e
        ...
        if not _is_fallback_enabled_for_call:
            if piper_error_encountered:
                ...
                raise PiperConfigError(f"Piper flow not attempted and fallback disabled for '{variable_name}'.")
        ...
        if secret_value_from_env:
            ...
            if piper_error_encountered:
                ...
                details = getattr(piper_error_encountered, 'error_details', None)
                status = getattr(piper_error_encountered, 'status_code', None)
                code = getattr(piper_error_encountered, 'error_code', None)
                if isinstance(piper_error_encountered, (PiperLinkNeededError, PiperAuthError, PiperConfigError)):
                    raise type(piper_error_encountered)(
                        f"{original_error_msg} Also, fallback env var '{env_var_to_check}' not found.",
                        status_code=status, error_code=code, error_details=details
                ...
                raise PiperConfigError(f"Could not retrieve credentials for '{variable_name}'. Piper not configured and environment variable '{env_var_to_check}' not set.")